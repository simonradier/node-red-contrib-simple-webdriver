import {
  ...
  Protocol,
  BrowserType,
} from "@critik/simple-webdriver";
import {
  checkIfOnline as checkIfReachable,
  REDAPI,
  replaceMustache,
} from "../utils";
...
export function NodeOpenBrowserConstructor(
  this: NodeOpenWeb,
  conf: NodeOpenBrowserDef
) {
  ...
  if (!conf.serverURL) {
    ...
    checkIfReachable(conf.serverURL)
      .then((result) => {
        if (result) {
  ...
  this.on("input", async (message: any, send, done) => {
    ...
    try {
      if (msg.browser) {
        if (!driverError) {
          const window = await msg.browser.getCurrentWindow();
          if (!conf.maximized)
            await window.setSize(
              parseInt(conf.width, 10),
              parseInt(conf.height, 10)
            );
          else if (!conf.headless) await window.maximize();
        }
        send(msg);
